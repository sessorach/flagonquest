...
- Went back to that Level 1 calibration once its win-rate numbers stopped holding up - the check that landed on DamageBonus+4 hadn't been run through the simulator's official method, and once it was, on-level fights read far too hard, not too easy. Also found the whole Roster's validated win-rate table (from a few days back) has gone stale since - several mechanics landed after it was measured (Card Techniques, the real AP/Armor economy, real turn order) and nobody re-checked it against them, so most Levels now read close to 100% instead of ~50%. Only Level 1 got fixed today. Added two new retune knobs alongside DamageBonus - AccuracyBonus and DefenseBonus - and found DefenseBonus (an enemy's own Parry/Dodge/Bodily/Mental, moved together) is the cleanest of the three for landing a win rate and a "Health left on a win" at the same target together, since it changes how fast the party kills the enemy instead of how hard the enemy hits back. Level 1's four archetypes now land around 68% win, about 4-5 rounds, and 68% party Health remaining on a win. Paused that pass right after, though - DamageBonus/AccuracyBonus/DefenseBonus are back to blank, so Level 1 reads off the plain spreadsheet-curve numbers again for now; the retune's still there in git history whenever it's worth picking back up.
- Found a real gap while digging into that pause: Harried (get attacked with your Parry or Dodge, take -1 to both until your own turn ends) wasn't modeled anywhere in the simulator - a base rule, not an optional Ability, so every fight this whole project has run was missing it. Wired it in on both sides and it moved the numbers a lot (Tier 1 vs Level 1 alone went from ~36% win/~7.6 rounds to ~91%/~5.8 once it's actually on). Also built a second, alternative way to construct an enemy - straight off the real PC formulas (Defense = 8 + Skill Total, Damage = a weapon-style base + Stat, Resist = Essence alone) instead of the simulator's own synthetic Level curve, using the Skill Total table already validated in ENEMY_ENCOUNTER_DESIGN.md. Turns out that once Harried is on, those plain, ungrounded numbers land close to the target win rate and Health cost on their own, with no extra bonus knob needed. Wired this in as the new Level 1 baseline for Hedge Knight/Marsh Archer/Skulking Footpad/Fen Warden, and the combat log now shows a target's current Harried count next to each attack.
- Swapped the win-rate calibration over to the real drafted party (Hilde/Browndog/Carrick/Sable) instead of the smoothed generic one - turns out these four are noticeably stronger fighters on their own, no teamwork needed yet, so the enemy side needs retuning to match rather than reusing the old numbers as-is. Gave the party a real base strategy too: attack twice if the AP's there (already true for everyone, no change needed), then focus fire on whichever enemy's hurt worst, then - if nobody's wounded yet, mostly turn 1 - go for whoever the whole party's already closest to together, instead of each PC chasing whatever's nearest to itself. Only shows up once movement's on (the old static-mode default already happened to focus-fire by list order), but there it's a real gain: win rate up from 92% to 98%, and the downed rate (does anyone actually hit 0 mid-fight) drops from 64% to 47%. Also caught a real ordering bug along the way - a PC's own gambling decision was reading a target's Harried count one stack too high, counting the attack's own soon-to-land stack against itself instead of just what came before it.
- Retuned Level 1 one more time with all of today's pieces actually in place together (real party, teamwork targeting, Harried) and locked it in: Hedge Knight/Marsh Archer/Skulking Footpad/Fen Warden's Health goes to 12 (`HealthBonus=2`), everything else stays the plain spreadsheet numbers, current Abilities unchanged. Lands at win~99%, ~5.6 rounds, ~72% party Health remaining on a win - and the variance is noticeably tighter than anything found earlier today, mostly because the party isn't scattering its own damage anymore.
