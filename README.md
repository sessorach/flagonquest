...
- Followed up with a real mixed party instead of 4 clones (`party.make_party_from`), a 2x2 party formation, a random 5-10m starting gap between front lines instead of opposite arena corners, and Wren's War Magic now actually dealing Fire damage (drawing on enemies' generally-weaker elemental Resist instead of Physical) - mixed reference-build parties land well below the Roster's own ~50%, since the Roster was calibrated for 4 identical smoothed PCs, not spikier named builds sharing a slot with a non-attacking healer; testing swaps in a lighter, more proportionate encounter showed a 2nd caster or 4th fighter both beating a dedicated healer (Beornhard, who heals ~25% of rounds or nearly always once someone's badly hurt) in this simplified model, where a steady trickle of healing loses out to more damage.
