...
| Resist (Physical) | 2.5/point *(was 2.8)* | Yes | New row — never had one, despite 7 current Masterwork items granting Resist. `Damage's weight (2) × expected hits of that type absorbed per encounter`, using Baseline's own combat math (~1.875 hits/player/encounter, any type) and the designer's stated damage-share mix — corrected from an initial pass that used enemy *headcount* share (3 in 4 Physical) as a stand-in for *damage* share; the elemental enemy in a relevant fight tends to be the actual main damage dealer, so the real Physical share is 2/3, not 3/4. |
| Resist (Fire) | 0.5/point *(was 0.375)* | Yes | Same formula, at Fire's corrected damage share (2/15) — an elemental-relevant encounter happens about half the time, Fire is twice as likely as each other element within that half, and within a Fire-relevant fight Fire now accounts for ~2/3 of that fight's damage (not just its headcount share) — see the design note on Fire being the default/likely element. |
| Resist (Frost / Brilliant / Shadow, each) | 0.25/point *(was 0.19)* | Yes | Same formula, at each of the other three elements' corrected damage share (1/15 each) — half Fire's rate, 1/10th of Physical's. |
