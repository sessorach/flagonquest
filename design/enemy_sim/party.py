...
- **Reference builds** (`Roster` FALSE) - named Level 1 characters, one
  ...
  profile against the validated Roster (see combat_sim.py's own note on
  what that showed for the three ranged builds below). Melee-attacking:
  Hilde, Browndog, Carrick, Jackal, Felix (Felix, Essence-primary with
  barely any Melee, reads as a weak attacker despite being a coherent
  character on paper - see the Weapon paragraph below for why, and
  Wren for the same Essence-primary concept actually built to attack
  through Sorcery instead). Ranged-attacking (see `Weapon` below):
  Sable (Archery/Light Bow), Rook (Acrobatics/Light Thrown), Wren
  (Sorcery/War Magic + Lance).

**`Weapon`** (blank for most rows) switches which Skill/Stat combo
governs a PC's own attack roll, Damage, and attack range - a real
capability this sim didn't have originally, when every PC's attack was
hardcoded to Melee regardless of build. A blank cell keeps that original
1H Heavy Melee default (Melee Skill Total, Damage = 4 + Body, no
...
features.csv sourcing. This is fully decoupled from
...
def _pc_dict(row, index, good_luck):
    ...
    skills = {k: v for k, v in row.items() if k not in
              ("Name", "Tier", "Agility", "Body", "Cunning", "Mind", "Essence", "Health", "Roster", "Notes", "Weapon")}
    ...
    # accuracy bonus, Damage 4 + Body, no attack_range - falls back to
    # MELEE_RANGE) unless `Weapon` names a ranged option in T.WEAPON, in
    # which case the attacking Skill/accuracy/Damage/Range all switch to
    # that weapon's own numbers - see T.WEAPON's own comment for the real
